...
### Development
To start a development server run `yarn run start`. This will start the json server on port 3030 as well as a react development server on port 3000.

### Production
Running a production ready server is archieved by running `yarn run run`. This will build the react code and the json server will statically server the build folder. 
